...
package App::Feedmailer {
    ...
    our $BUGREPORT = 'inne <inne@sdf-eu.org>';
    ...
    sub get_feed {
        my ( $uri, $params ) = @_;
        $logger->info( sprintf( "Downloading feed %s…", $uri ) );
        my $feed;
        eval {
    ...
    sub find_feeds {
        ...
        $logger->info( sprintf( "Searching feeds at %s…", $uri ) );
        my @feeds;                    # = XML::Feed->find_feeds( $uri );
        eval {
            my $response = download(@_);
            @feeds =
              Feed::Find->find_in_html( \$response->decoded_content, $uri );
        };
        $@ && $logger->warn( sprintf( "$uri: %s", $@ ) );
        return @feeds;
    }
...
m/^[[:alnum:]]+:\/[[:alnum:]]*\/[[:alnum:]\._~:\/\?#\[\]@!\$\&'\(\)*+,;=% \-]+$/i;
    ...
    sub ig {
        ...
        for ( @{ $args->{whitelist} } ) {
            if ( $string =~ m/$_/i ) {
                $logger->info(
                    sprintf( "Whitelist: %s matched %s", $string, $_ ) );
                $ig   = 0;
                $re   = $_;
                $list = "whitelist";
                last;
        ...
        for ( @{ $args->{blacklist} } ) {
            if ( $string =~ m/$_/i ) {
                ...
                $logger->info(
                    sprintf( "Blacklist: %s matched %s", $string, $_ ) );
                $ig   = 1;
                $re   = $_;
                $list = "blacklist";
                last;
        ...
        $logger->info(
            sprintf(
                "ignore flag is '%i' for '%s' (in list %s: %s)",
                $ig, $string, $list, $re
            )
        );
...
=cut
    ...
    sub force_secure_scheme {
        ...
        if ( $uri->scheme eq "ftp" ) {
            $uri->scheme("ftps");
            $logger->info(
                sprintf( "Using secure scheme %s for %s", $uri->scheme, $uri )
            );
        }
...
=cut
    ...
    sub clean_up {
        ...
        if ($s) {
            ...
            $s = HTML::Strip->new()->parse( $s || "" );    # TODO/FIXME
                #$s = Text::Trim::ltrim($s);
                #$s = Text::Trim::rtrim($s);
            $s =~ s/^[[:space:]]+//;
            $s =~ s/[[:space:]]+$//;
...
=cut
    ...
    sub file2list {
        ...
        $logger->info( sprintf( "Loading file: %s", $file ) );
