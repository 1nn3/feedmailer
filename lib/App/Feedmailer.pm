...
package App::Feedmailer {
    ...
    $logger->info("Application %s Version %s started", $NAME, $VERSION);

    our $PACKAGE_STRING = "$NAME $VERSION";

    # ~/.feedmailer
    our $HOME = File::Spec->catfile(File::HomeDir->my_home(), '.feedmailer');
    ...
    our $tt;
    our $tt_cfg = { INCLUDE_PATH => get_file( "templates" ) };
    ...
    $COOKIE_FILE = get_file( $COOKIE_FILE );
    ...
    # identify
    $ua->agent("");                 # $ua->agent($PACKAGE_STRING);
    $ua->from("");                  # $ua->from($BUGREPORT);
    ...
    sub load_config {
        ...
        $logger->info(sprintf("Read the configuration: %s", $path));
    ...
    sub save_cache {
        ...
        $path //= get_file( $CACHEFILE );

        $logger->info(sprintf("Save cache: %s", $path));
    ...
    sub load_cache {
        ...
        $logger->info(sprintf("Load cache: %s", $path));

        my $fh = IO::File->new();
        if ( !$fh->open( $path, "r" ) ) {
            $logger->warn(sprintf("$!: %s", $path));
        }
  ...
  #*
    ...
    sub get_feed {
        my ( $uri, $params ) = @_;
        $logger->info(sprintf("Download feed: %s", $uri));
        my $feed;
        eval {
        ...
        $@ && $logger->warn(sprintf("$uri: %s", $@));
        return $feed;
    }

    sub find_feeds {
        my ( $uri, $params ) = @_;    # $uri must be a string!
        $logger->info(sprintf("Run find feed: %s", $uri));
        my @feeds;                    # = XML::Feed->find_feeds( $uri );
        eval {
            my $response = download(@_);
            @feeds = Feed::Find->find_in_html( \$response->decoded_content,
                $uri );
        };
        $@ && $logger->warn(sprintf("$uri: %s", $@));
        return @feeds;
    ...
    sub ig {
        ...
        for ( @{ $args->{whitelist} } ) {
            if ( $string =~ m/$_/i ) {
                $logger->info(sprintf("Whitelist: %s matched %s", $string, $_));
                $ig   = 0;
                $re   = $_;
                $list = "whitelist";
                last;
        ...
        for ( @{ $args->{blacklist} } ) {
            if ( $string =~ m/$_/i ) {
                ...
                $logger->info(sprintf("Blacklist: %s matched %s", $string, $_));
                $ig   = 1;
                $re   = $_;
                $list = "blacklist";
                last;
        ...
        $logger->info(sprintf("ignore flag is '%i' for '%s' (in list %s: %s)",$ig,$string,$list,$re));

        if ( get_cfg_val( "invert_ig", 0 ) ) {
            $logger->info("useing inverted ignore flag…");
            $ig = !$ig;
        }
    ...
    sub download {
        ...
        eval {
            ...
            if ( $params->{proxy} ) {
                ...
                $logger->info(sprintf("Using PROXY %s for URI %s",$params->{proxy},$uri));
            }
...
=cut
    ...
    sub force_secure_scheme {
        ...
        if ( $uri->scheme eq "ftp" ) {
            $uri->scheme("ftps");
            $logger->info(sprintf("Using secure scheme %s for %s", $uri->scheme, $uri));
        }
